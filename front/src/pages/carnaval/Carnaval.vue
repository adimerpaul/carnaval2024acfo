...
export default {
  ...
  methods: {
    lineasGet () {
      api.get('lineas', { timeout: 5000 })
        .then((res) => {
          this.polyline.latlngs = res.data
          // Guardamos la data en localStorage
          localStorage.setItem('lineas', JSON.stringify(res.data))
        })
        .catch((error) => {
          console.error('Error en la petición de lineas, se intenta cargar desde localStorage', error)
          const cachedLineas = localStorage.getItem('lineas')
          if (cachedLineas) {
            this.polyline.latlngs = JSON.parse(cachedLineas)
          } else {
            console.error('No hay datos en localStorage para lineas')
          }
        })
    },
    filterFn (val, update) {
    ...
    getDancers () {
      this.loading = true
      this.$axios.get('dancers', { timeout: 5000 })
        .then((res) => {
          this.dancers = res.data
          this.dancerAll = res.data
          // Guardamos la data en localStorage
          localStorage.setItem('dancers', JSON.stringify(res.data))
          this.getCogs()
        })
        .catch((error) => {
          console.error('Error en la petición de dancers, se intenta cargar desde localStorage', error)
          const cachedDancers = localStorage.getItem('dancers')
          if (cachedDancers) {
            this.dancers = JSON.parse(cachedDancers)
            this.dancerAll = JSON.parse(cachedDancers)
          }
          this.$q.notify({
            color: 'negative',
            position: 'top',
            message: error.response?.data?.message || 'Error al obtener dancers',
            icon: 'report_problem'
          })
        })
        .finally(() => {
